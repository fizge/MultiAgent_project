...
\begin{itemize}
    ...
    \texttt{ladronAvistado}, \texttt{bloquearSalida} y busquedaCoordinada.
...
\begin{itemize}
    ...
    \item \textbf{Método público \texttt{ComprobarEstadoCofre()}} que devuelve \texttt{true} si el cofre sigue intacto, usado por \texttt{CheckChestBehaviour} respetando la arquitectura TM.
...
Sensor nuevo en la práctica 2. Detecta emisores de ruido (\texttt{NoiseEmitter}) en un radio configurable, distingue al ladrón (tag \texttt{Ladron}) de otros guardias (tag \texttt{Esqueleto}) y aplica una imprecisión configurable a la posición percibida. Dispara \texttt{OnLadronEscuchado}, \texttt{OnEsqueletoEscuchado} y sus correspondientes eventos de pérdida.
...
\begin{itemize}
    ...
    \item \texttt{INFORM} de búsqueda coordinada: cuando un patrullero pierde al ladrón, en lugar de usar Contract Net emite directamente un \texttt{INFORM} a todos los patrulleros con la última posición conocida. No hay negociación ni ganador único: todos los receptores (incluido el emisor, porque puede ser él el que está más cerca) calculan autónomamente su sector de búsqueda y se dirigen a él. Este uso del \texttt{INFORM} fuera del flujo Contract Net es deliberado: la coordinación emerge de las posiciones relativas sin necesidad de propuestas.
\end{itemize}
...
\paragraph{Bloqueo de la zona de salida} Al detectar el cofre robado por visión directa (\texttt{OnCofresDesaparecido}), un patrullero lanza \texttt{bloquearSalida} con la posición de la \texttt{ZonaSalida}. El más cercano va allí y queda patrullando la salida. Este efecto ocurre en paralelo al modo alerta: ambos coexisten porque uno surge del INFORM broadcast del Contract Net y el otro de la percepción directa.
