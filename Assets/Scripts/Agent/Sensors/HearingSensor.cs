...
public class HearingSensor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ladronDetectado  && !ladronEscuchadoAntes)  OnLadronEscuchado?.Invoke(PosicionRuidoLadron);
        if (!ladronDetectado &&  ladronEscuchadoAntes)  OnLadronNoEscuchado?.Invoke();

        if (esqueletoDetectado  && !esqueletoEscuchadoAntes) OnEsqueletoEscuchado?.Invoke(PosicionRuidoEsqueleto);
        if (!esqueletoDetectado &&  esqueletoEscuchadoAntes) OnEsqueletoNoEscuchado?.Invoke();
